...
def load_articles():
    ...
    :return:
    """
    filename = 'data/cluster_article.pkl'
    ...
    print("正在分词...")
    with open('C:\\Users\\ZhouXunZhe\\Desktop\\ift_cluster_given_fudandm2023.jsonl', 'r', encoding="utf-8") as f:
        for line in f:
            article = json.loads(line)['input']
            articles.append(" ".join([word for word in jieba.cut(article) if word not in stop_words]))
...
def out():
    ...
    articles = load_articles()
    X, vectorizer = transform(articles, n_features=500)
    score = train(X, vectorizer, true_k=50, show_label=True) / len(articles)
    print(score)
